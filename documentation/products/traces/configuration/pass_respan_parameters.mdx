...
For the complete list of available parameters, see the [Respan API reference](https://docs.keywordsai.co/api-endpoints/observe/logs/request-logging-endpoint).
...
async function simpleTask() {
    return await keywordsAI.withTask(
        { name: 'simple_task' },
        async () => {
            return await keywordsAI.withRespanSpanAttributes(
                async () => {
...
1. Navigate to the [Traces page](https://platform.keywordsai.co/platform/traces) in your Respan dashboard
