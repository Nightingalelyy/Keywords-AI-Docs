...
To use [Respan parameters](/api-endpoints/develop/gateway/chat-completions#keywords-ai-parameters), you can pass them in the `extra_body` parameter if you're using the OpenAI SDK.

<Info>
**Environment Switching**: Respan doesn't support an `env` parameter in API calls. To switch between environments (test/production), use different API keys - one for your test environment and another for production. You can manage these keys in your [API Keys settings](https://platform.keywordsai.co/platform/api/api-keys).
</Info>
