...
To use [Respan parameters](/api-endpoints/develop/gateway/chat-completions#keywords-ai-parameters), you can pass them in the `metadata` parameter. In the above example, `customer_identifier` is a Respan parameter. These parameters will take precedence over the Anthropic parameters if they are conflicting.
