...
client = OpenAI(
    ...
    api_key="YOUR_KEYWORDS_AI_API_KEY",  # Get from Respan dashboard
)
```
...
const client = new OpenAI({
  ...
  apiKey: process.env.KEYWORDS_AI_API_KEY,  # Get from Respan dashboard
});
```
...
func main() {
    client := openai.NewClient(
        ...
        option.WithAPIKey("YOUR_KEYWORDS_AI_API_KEY"), // Get from Respan dashboard
    )
}
```
</CodeGroup>
...
[Respan parameters](/api-endpoints/develop/gateway/chat-completions#keywords-ai-parameters) can be passed for better handling and customization.
...
<CodeGroup>
```plain Setup
1. Go to [Respan Providers](https://platform.keywordsai.co/platform/api/providers)
